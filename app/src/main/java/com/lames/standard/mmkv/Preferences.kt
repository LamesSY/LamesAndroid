...
inline fun <reified T : Parcelable> MMKV.parcelable(key: String? = null, defaultValue: T?): ReadWriteProperty<Any, T> =
...
fun SharedPreferences.spBoolean(key: String, defaultValue: Boolean = false): ReadWriteProperty<Any, Boolean> =
    spDelegate(key, defaultValue, SharedPreferences::getBoolean) { _, value -> edit { putBoolean(key, value) } }

fun SharedPreferences.spInt(key: String, defaultValue: Int = 0): ReadWriteProperty<Any, Int> =
    spDelegate(key, defaultValue, SharedPreferences::getInt) { _, value -> edit { putInt(key, value) } }

fun SharedPreferences.spLong(key: String, defaultValue: Long = 0L): ReadWriteProperty<Any, Long> =
    spDelegate(key, defaultValue, SharedPreferences::getLong) { _, value -> edit { putLong(key, value) } }

fun SharedPreferences.spFloat(key: String, defaultValue: Float = 0.0F): ReadWriteProperty<Any, Float> =
    spDelegate(key, defaultValue, SharedPreferences::getFloat) { _, value -> edit { putFloat(key, value) } }

fun SharedPreferences.spString(key: String, defaultValue: String? = null): ReadWriteProperty<Any, String?> =
    spNullableDefaultValueDelegate(key, defaultValue, SharedPreferences::getString) { _, value -> edit { putString(key, value) } }
//endregion