...
const val HIDE_UI_FLAG = (View.SYSTEM_UI_FLAG_LOW_PROFILE or View.SYSTEM_UI_FLAG_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE
...
fun Activity.hideSystemUI() = runOnUiThread { window?.decorView?.let { it.systemUiVisibility = HIDE_UI_FLAG } }
fun Activity.showSystemUI() = runOnUiThread { window?.decorView?.let { it.systemUiVisibility = SHOW_UI_FLAG } }

fun DialogFragment.hideSystemUI() = dialog?.window?.decorView?.let { it.systemUiVisibility = HIDE_UI_FLAG }
fun DialogFragment.showSystemUI() = dialog?.window?.decorView?.let { it.systemUiVisibility = SHOW_UI_FLAG }
...
fun PopupWindow.show(anchorView: View, xOffset: Int, yOffset: Int, gravity: Int = Gravity.NO_GRAVITY) = apply {
    isFocusable = false
